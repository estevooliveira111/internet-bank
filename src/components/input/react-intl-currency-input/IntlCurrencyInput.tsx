...
const IntlCurrencyInput = ({
  ...
  const safeConfig = useMemo(
    () => () => {
      const {
        formats: {
          number: {
            [currency]: { maximumFractionDigits = MAXIMUM_FRACTION_DIGITS },
          },
        },
      } = config
  ...
  const normalizeValue = (entry: string | number): number => {
    ...
    if (typeof entry === 'string') {
      ...
      safeValue = Number.isInteger(safeValue)
        ? Number(safeValue) * 10 ** maximumFractionDigits
  ...
  const handleFocus = (event: {
    target: { select: () => void; value: number }
  }) => {
    if (autoSelect) {
  ...
  const handleKeyUp = (event: { key: any; keyCode: any }) =>
